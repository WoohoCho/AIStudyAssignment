...
# 분류를 위한 Decision Tree 모델 생성
dt_classifier = DecisionTreeClassifier()

# 회귀를 위한 Decision Tree 모델 생성
...
# 회귀에 사용할 데이터 준비
X_regression = df.drop('trestbps', axis=1)
y_regression = df['trestbps']

# Decision Tree 분류 모델 학습
dt_classifier.fit(X_classification, y_classification)

# Decision Tree 회귀 모델 학습
dt_regressor.fit(X_regression, y_regression)

# 모델 평가 및 활용
# 분류 모델의 경우 accuracy_score 등으로 평가하고, 회귀 모델의 경우 평가 지표로 MSE나 R-squared 등을 활용할 수 있습니다.
...
# 분류 모델 평가
y_pred_classification = dt_classifier.predict(X_classification)
accuracy = accuracy_score(y_classification, y_pred_classification)
print(f"Accuracy: {accuracy}")

# 분류 보고서 출력
print(classification_report(y_classification, y_pred_classification))
...
# 회귀 모델 평가
y_pred_regression = dt_regressor.predict(X_regression)
